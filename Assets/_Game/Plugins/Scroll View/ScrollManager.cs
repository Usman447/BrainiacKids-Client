...
namespace ScrollView
{
    ...
    public class ScrollManager : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        #region Fields
        ...
        public bool useAutomaticLayout = true;
        ...
        public bool useInfiniteScrolling = false;
        ...
        [SerializeField] private bool useSelectingAndSwipping = false;
        ...
        private bool ValidConfig
        {
            get
            {
                ...
                if (pagination != null)
                {
                    ...
                    {
                        Debug.LogError("The number of Toggles should be equivalent to the number of Panels. There are currently " + numberOfToggles + " Toggles and " + NumberOfPanels + " Panels. If you are adding Panels dynamically during runtime, please update your pagination to reflect the number of Panels you will have before adding.", gameObject);
                        valid = false;
                    }
                }
                if (snapSpeed < 0)
                {
                    Debug.LogError("Snapping speed cannot be negative.", gameObject);
                    valid = false;
                }
        ...
        private void Start()
        {
            if (ValidConfig)
            {
                //startingPanel = NumberOfPanels - 1;
                Setup();
            }
            else
            {
        ...
        public GameObject Add(GameObject panel, int index)
        {
            ...
            {
                Debug.LogError("Index must be an integer from 0 to " + NumberOfPanels + ".", gameObject);
                return null;
            }
            else if (!useAutomaticLayout)
            {
                Debug.LogError("Automatic Layout must be enabled for content to be dynamically added during runtime.");
                return null;
            }
        ...
        public void Remove(int index)
        {
            if (NumberOfPanels == 0)
            {
                Debug.LogError("There are no panels to remove.", gameObject);
                return;
            }
            else if (index < 0 || index > (NumberOfPanels - 1))
            {
                Debug.LogError("Index must be an integer from 0 to " + (NumberOfPanels - 1) + ".", gameObject);
                return;
            }
            else if (!useAutomaticLayout)
            {
                Debug.LogError("Automatic Layout must be enabled for content to be dynamically removed during runtime.");
                return;
            }
