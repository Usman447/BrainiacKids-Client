...
namespace Network
{
    ...
    public abstract class Peer
    {
        ...
        protected void HandleData(object _, DataReceivedEventArgs _event)
        {
            MessageHeader header = (MessageHeader)_event.DataBuffer[0];

            Message message = Message.CreateRaw();
            message.PrepareForUse(header, _event.Amount);

            if (message.SendMode == MessageSendMode.Reliable)
            {
                if (_event.Amount < 3) // Reliable messages have a 3 byte header, if there aren't that many bytes in the packet don't handle it
                    return;

                if (_event.FromConnection.ReliableHandle(Converter.ToUShort(_event.DataBuffer, 1)))
                {
                    Array.Copy(_event.DataBuffer, 1, message.Bytes, 1, _event.Amount - 1); // We've already established that the packet contains at least 3 bytes, and we always want to copy the sequence ID over
                    messagesToHandle.Enqueue(new MessageToHandle(message, header, _event.FromConnection));
                }
            }
            else
            {
                if (_event.Amount > 1) // Only bother with the array copy if there is more than 1 byte in the packet (1 or less means no payload for a reliably sent packet)
                    Array.Copy(_event.DataBuffer, 1, message.Bytes, 1, _event.Amount - 1);
